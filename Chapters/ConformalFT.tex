...
\subsection[The conformal group in greater or equal to three dimensions]{The conformal group in $\md>2$}
...
\bse 
2 \md x^{\prime +} \md x^{\prime,-} = 2 \frac{\partial x^{\prime -}}{\partial x^-} \frac{\partial x^{\prime +}}{\partial x^+} \md x^- \md x^+ 
\ese 
...
\be
z = x^+ = \frac{1}{\sqrt{2}} (\sigma + \tau ),\quad \bar{z} = x^- = \frac{1}{\sqrt{2}} (\sigma-\tau).
\ee 
...
\bse 
\xi(z) \frac{\partial }{\partial z} = \sum_n a_n z^{n+1} \partial_z \equiv - \sum_n a_n L_n
\ese 
with the \emph{generators of conformal transformations}
\be 
L_n = - z^{n+1} \partial_z 
\ee
\todo{Check whether this sign is correct}
\begin{mybox}{Witt algebra}
	...
	[L_n,L_m] = (n-m) L_{n+m},\quad \text{also} \quad [\bar{L}_n,\bar{L}_m] = (n-m) \bar{L}_{n+m}.
	\ee
\end{mybox}
\subsubsection{Global subgroup - Möbius}
...
\bse 
\{L_0,L_1,L_{-1} \}\quad \text{or} \quad \{\bar{L}_0,\bar{L}_1,\bar{L}_{-1} \}..
\ese 
...
\bse 
z \rightarrow \frac{az+b}{cz+d} \quad \text{with} \quad ad-bc=1
\ese 
...
\bse 
\phi^\prime(\lambda z,\bar{\lambda}\z) = \underbrace{r^{-(h+\h)}}_{\text{dilatations}} \underbrace{e^{-i \theta(h-\bar{h}) } }_{\text{rotations}} \phi(z,\bar{z}) 
\ese 
...
\subsubsection{Field transformation under a general conformal transformation}
\begin{mybox}{Primary field}
...
\delta \phi = - h \frac{\partial \xi}{\partial z} \phi - \xi \frac{\partial \phi}{\partial z} - \h \frac{\partial \bar{\xi}}{\partial \z} \phi - \bar{\xi} \frac{\partial \phi}{\partial \z}.
\ee 
...
\bse 
\partial_\mu T^{\mu \nu} = 0 \quad \text{and} \quad T^\mu_\mu = 0.
\ese 
...
(x_1-x_2)^\mu = u \vec{n}^\mu_1 - r \vec{n}^\mu_2 \rightarrow u \vec{n}^\mu_1, \; (x_1-x_2)^2=u^2 \vec{n}^2_1 = u^2
 ...
 \bse 
 \vec{n}^\mu = \begin{pmatrix}
 	\cos \theta \\
 	\sin \theta \\
 \end{pmatrix},\quad 
z=r e^{i\theta},\; r =\abs{z} \quad \forall \theta.
 \ese 
 \subsubsection{Hermitian conjugation in $2\md$ radial quantization}
 For $h=\h$, $\Delta = 2h$, such that for scalar field we have
 \bse 
 \mO(z,\z)^\dagger = \frac{1}{(z \z)^{2h}} \mO(\frac{1}{\z},\frac{1}{z} ), \quad \frac{1}{\z} = r^{-1} e^{i\theta}.
 \ese 
 The generalization for fields with spin reads
 \be 
 \label{eq:cfthermitianconjugation2d}
 \phi(z,\z)^\dagger = \frac{1}{z^{2 \bar{h}} } \frac{1}{\z^{2 h}} \phi(\frac{1}{\z}, \frac{1}{z}).
 \ee 
 ...
 \phi(z,\z) = \sum_{n,m} \frac{\phi_{nm}}{z^{n+h} \z^{m+\h}}.
 \ee 
 ...
 \bse 
 \ket{\phi}= \sum_{n,m} \frac{\phi_{nm}}{z^{n+h} \z^{m+\h} } |_{z,\z=0} \ket{0}.
 \ese 
 This expression obviously blows up, at the quantum level we therefore have to enforce
 \be 
 \phi_{nm} \ket{0}=0 \quad \text{for } n+h >0 \& m+\h >0.
 \ee 
 Thus
 
 \be 
 \ket{\phi} = \phi_{-h,-\h} \ket{0}, \; \bra{\phi}= \bra{0}(\phi_{-h,-\h})^\dagger =\bra{0}\phi_{h,\h}
 \ee 
...
\subsection{The Operator Product Expansion in $\md=2$}
...
\subsubsection{Note on Noether symmetries in $\md=2$}
Recall that we had in higher dimensions, that the conformal Killing vector $\xi^\nu \partial_\nu$ has a corresponding Noether current
\be 
\xi^\nu \partial_\nu \text{ conformal Killing vector }  \rightarrow j^\mu = T^{\mu \nu}\xi_\nu \text{ conserved current } \rightarrow Q_3 = \int T^{0 \nu} \xi_\nu \md^3 x \text{ conserved charge}.
\ee
...
\be 
[Q_\xi, \phi(w)] = \frac{1}{2 \pi i} \oint_{\abs{z-2} = R,\text{ anti-clockwise}} \xi(z) T(z) \md z,
\ee 
...
\begin{mybox}{Claim}
	...
	\be 
	\label{eq:cftOPE2d}
	T(z)\phi(w) = \frac{h}{(z-w)^2} \phi(w) + \frac{1}{z-w} \partial_w \phi(w) + \dots 
	\ee 
	...
	\bse 
	\text{ transformation of primary } \leftrightarrow \text{ OPE of primary with }T
	\ese 
...
using 
\bse 
\frac{1}{(z-w)^2} =-\partial_z \frac{1}{z-w}.
\ese 
...
\begin{mybox}{Ward-Takahashi in $\md=2$}
 ...
 \be
 S = -\frac{1}{4 \alpha^\prime} \int \md^2X \partial_a X \partial^a X.
 \ee 
 ...
 \be 
 X = x - \sqrt{2 \alpha^\prime} \alpha_0 t - i \sqrt{\half\alpha^\prime} \sum_{n\neq 0} \left[ \frac{\alpha_n}{n} e^{-i n (t+\sigma)} + \frac{\bar{\alpha}_n}{n} e^{-i n (t-\sigma)} \right].
 \ee 
 ...
 \be 
 p = \sqrt{\frac{2}{\alpha^\prime}} \alpha_0 
 \ee 
 ...
 \begin{enumerate}
 	...
 	\bse 
 	e^{-i (\tau +\sigma)} \rightarrow e^{-\tau} e^{-i\sigma} = \frac{1}{r e^{i \sigma}} = \frac{1}{z}.
 	\ese 
 \end{enumerate}
 We then have
 \be 
 X = x + i \alpha^\prime p \ln(\abs{z}) - i \sqrt{\frac{\alpha^\prime}{2}} \sum_{n\neq 0} \left[\frac{\alpha_n}{2} z^{-n} + \frac{\bar{\alpha}_n}{n} \z^{-n} \right]
 \ee 
 ...
 \be 
 \partial_z X = i \sqrt{\frac{\alpha^\prime}{2}} \sum_n \frac{\alpha_n}{z^{n+1}},\; \partial_{\bar{z}} X= i \sqrt{\frac{\alpha^\prime}{2}} \sum_n \frac{\bar{\alpha}_n}{\z^{n+1}}.
 \ee 
 ...
 \be
 \Pi = \frac{\partial L}{\partial \dot{X}} = \frac{1}{2 \pi \alpha^\prime} \dot{X},
 \ee 
 which is the canonically conjugate momentum to $X$, such that
 \be 
 [X(t,\sigma), \dot{X}(t,\sigma^\prime)] = 2 \pi \alpha^\prime i \delta(\sigma-\sigma^\prime).
 \ee 
 This gives the commutators for the modes
 \be 
 [x,p] = i,\; [\alpha_m,\alpha_n] = m \delta_{n+m,0}, \; [\bar{\alpha}_m, \bar{\alpha}_n] = m \delta_{n+m,0}.
 \ee 
 ...
 \be 
 H= \int \md \sigma (\Pi \dot{X} - L) = \frac{\alpha^\prime}{2} p^2 + \sum_{n>0} ( \alpha_{-n} \alpha_n + \bar{\alpha}_{-n} \bar{\alpha}_n).
 \ee 
 ...
 \be 
 \alpha_m \ket{0;k} =0 \quad \bar{\alpha}_m \ket{0;k} = 0 \quad \forall m >0.
 \ee 
 ...
 \bse 
 \alpha^{n_1}_{-1} \alpha^{n_2}_{-2} \dots \bar{\alpha}^{\bar{n}_1}_{-1} \bar{\alpha}^{\bar{n}_2}_{-2} \dots \ket{0;k}.
 \ese 
